class Campaign {
    ...
    if (body) {
      if (Array.isArray(body)) {
        body.map((campaign, index) => {
          if (index) this.campaign = [...this.campaign, campaign];
  ...
  getData() {
    ...
    if (this.campaignName) data.campaignName = this.campaignName;
    if (this.campaignBrand) data.campaignBrand = this.campaignBrand;
    if (this.campaignTarget)
      data.campaignTarget = Array.isArray(this.campaignTarget)
        ? this.campaignTarget
        : [this.campaignTarget];
    if (
      this.campaignDiscountValue &&
      !Number.isNaN(parseFloat(this.campaignDiscountValue))
    )
    ...
    if (
      ...
      else data.isDiscountPercentage = this.isDiscountPercentage;
    }
    if (
      this.isActive === true ||
      this.isActive === false ||
      ...
      if (this.isActive === 'true') data.isActive = true;
      else if (this.isActive === 'false') data.isActive = false;
      else data.isActive = this.isActive;
    }
  ...
  getRaw() {
    let data = {};
    if (this.campaign) return this.campaign;
    if (this._id) data.campaignId = this._id;
    if (this.campaignName) data.campaignName = this.campaignName;
    if (this.campaignBrand) data.campaignBrand = this.campaignBrand;
    if (this.campaignTarget)
      data.campaignTarget = Array.isArray(this.campaignTarget)
